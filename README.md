# Simple Create
Simple Create is a modpack intends on creating a relatively simple and fun environment centered on the [Create](https://modrinth.com/mod/Xbc0uyRg) mod in which to play Minecraft with one's friends in. For this reason a client and server versions of the modpack are provided. This modpack does make a few changes to some of Minecraft's core systems such as enchanting and exploration. If you do not like these changes feel free to disable them or remove the mods that make them.

This modpack is focused on create a simple, light and fun environment to play Minecraft with your friends in, you are therefore allowed to modify and redistribute the modpack as you see fit, please do credit Gljue, who made the icon, if you do decide to keep that aspect of the modpack.

## Mods that you may not like
The [Enchancement](https://modrinth.com/mod/6hN1V6wJ) mod, while fun, rebalances enchanting completely. Some players don't like these changes, however the mod makes up for the hard nerfs to enchanting with new, fun and interesting enchantements, however if it's not for you feel free to remove it.

Mods that rework how Minecraft generates its terrain may also be something you do not enjoy. However these do provide new and fresh environments for you to explore, so I suggest you give them a chance.

There are also some client side mods that you may find annoying, feel free to remove these if you do.

At the end of the day you need to find what works for you and your friends.

## Recommendations
Some things don't have to be changed with mods. There are a lot of cool datapacks out there, I'd check out [Vanilla Tweak](https://vanillatweaks.net/), they have a lot of good datapacks and crafting tweaks that work with vanilla and modded Minecraft, they also have really cool resource packs.

If you want something out of this world I'd also check out [Ad Astra](https://modrinth.com/mod/3ufwT9JF) and all of its extensions, as it adds space exploration to Minecraft and can be integrated with the [Create](https://modrinth.com/mod/Xbc0uyRg) mod. This might become a part of the modpack one day if we decide.

I'd also check out [Create: Aeronautics]( https://discord.gg/createaeronautics) which is currently unfinished, but will add physics based contraptions to [Create](https://modrinth.com/mod/Xbc0uyRg) and it'll be definitely added to this modpack once released.

Again these are recommendations and this modpack is meant to serve as inspiration or a jumping off point for you and your friends, so that you can play Minecraft in a way you want to.

## Hosting
This modpack uses [QuiltMC](https://quiltmc.org/en/) as the modloader. Please see their [website](https://quiltmc.org/en/) for details on how to install the client and/or server. If you're using a custom launcher like the [ATLauncher](https://atlauncher.com/), please refer to your launchers instructions on how to install modpacks. Most commonly used launchers support the Modrinth modpack format.

For the server admin's convenience  we have created a separate modpack version which removes all client-side only mods and adds a few useful server utilities, like [Discord Integration](https://modrinth.com/plugin/rbJ7eS5V) and [Fast Backups](https://modrinth.com/mod/ZHKrK8Rp).

Please do note that [Simple Voice Chat](https://modrinth.com/plugin/9eGKb6K1) requires extra configuration.

We've also included a mod list in this description where you can find all the mods we've used, whether they are needed on the client or server and whether they are ready for 1.20.

## 1.20
We plan on updating to 1.20 as soon as all of the mods in this pack support it. For a full list of mods and whether they support 1.20 see bellow.

## Conclusion and Credits
Big thanks once again to Gljue, who was too lazy to create a Modrinth account, and who create the modpack's icon and helped compile all the mods.

I'd also like to say once again that this modpack is meant to serve as an inspiration, so that you can fine-tune it in order to play the game you want to play. If you don't like something about this modpack feel free to change it.

Enjoy the modlist!

### Modlist
Dependancies not included.

|  Mod Name and Link | Client/Server Info | Ready for 1.20 Upgrade |
| ---------------------| :-----------------: | :-----------------------: |
| [Origins](https://modrinth.com/mod/3BeIrqZR) |  Client and Server | ✅ |
| [Enchancement](https://modrinth.com/mod/6hN1V6wJ) | Client and Server |  |
| [Impaled](https://modrinth.com/mod/AiHAhIP5) | Client and Server | |
| [Rat's Mischief](https://modrinth.com/mod/Jjc1149h) | Client and Server | |
| [Requiem](https://modrinth.com/mod/bYmqMxlQ) | Client and Server  | |
| [Farmer's Delight](https://modrinth.com/mod/4EakbH8e)  | Client and Server | ✅ |
| [Simple Voice-Chat](https://modrinth.com/plugin/9eGKb6K1) | Client and Server | ✅|
| [Create](https://modrinth.com/mod/Xbc0uyRg) | Client and Server | ✅|
| [Create: Steam&Rails](https://modrinth.com/mod/ZzjhlDgM) | Client and Server | ✅  |
| [Create: Slice&Dice](https://modrinth.com/mod/GmjmRQ0A) | Client and Server | ✅ |
| [Create: Enchantment Industry](https://modrinth.com/mod/AEZO385x) | Client and Server |  |
| [Create: Big Cannons](https://modrinth.com/mod/GWp4jCJj) | Client and Server | |
| [Better Nether](https://modrinth.com/mod/MpzVLzy5) | Client and Server | ✅|
| [Better End](https://modrinth.com/mod/gc8OEnCC) | Client and Server | ✅|
| [Regions Unexplored](https://modrinth.com/mod/Tkikq67H) | Client and Server | ✅|
| [Oh the Biomes You'll Go](https://modrinth.com/mod/uE1WpIAk) | Client and Server | |
| [Traveller's Backpacks](https://modrinth.com/mod/rlloIFEV) | Client and Server | ✅|
| [Carry On](https://modrinth.com/mod/joEfVgkn) | Client and Server | ✅ |
| [Supplementaries](https://modrinth.com/mod/fFEIiSDQ) | Client and Server | ✅ |
| [Appleskin](https://modrinth.com/mod/EsAfCjCV) | Client or Server | ✅ |
| [Jade](https://modrinth.com/mod/nvQzSEkH) | Client or Server | ✅ |
| [Jade Add-ons](https://modrinth.com/mod/fThnVRli) | Client or Server | |
| [Starlight](https://modrinth.com/mod/H8CaAYZC) | Client or Server | ✅ |
| [Memory Leak Fix](https://modrinth.com/mod/NRjRiSSD) | Client or Server | ✅ |
| [Debugify](https://modrinth.com/mod/QwxR6Gcd) | Client or Server | ✅ |
| [Smooth Boot](https://modrinth.com/mod/FWumhS4T) | Client or Server | (not required for 1.20) |
| [No Chat Reports](https://modrinth.com/mod/qQyHxfxd) | Client or Server | ✅ |
| [Concurrent Chunk Management Engine](https://modrinth.com/mod/VSNURh3q) | Server Only | | ✅ |
| [YUNG's Better Dungeons](https://modrinth.com/mod/o1C1Dkj5) | Server Only | ✅ |
| [YUNG's Better Mineshafts](https://modrinth.com/mod/HjmxVlSr) | Server Only | ✅|
| [YUNG's Better Strongholds](https://modrinth.com/mod/kidLKymU) | Server Only | ✅|
| [YUNG's Better Ocean Monuments](https://modrinth.com/mod/3dT9sgt4) | Server Only |✅ |
| [YUNG's Better Nether Fortresses](https://modrinth.com/mod/Z2mXHnxP)  | Server Only | ✅|
| [YUNG's Better Desert Temples](https://modrinth.com/mod/XNlO7sBv)  | Server Only |✅ |
| [YUNG's Better Witch Huts](https://modrinth.com/mod/t5FRdP87)  | Server Only |✅ |
| [YUNG's Better Bridges](https://modrinth.com/mod/Ht4BfYp6)  | Server Only |✅ |
| [YUNG's Extras](https://modrinth.com/mod/ZYgyPyfq) | Server Only |✅ |
| [Fast Back-Ups](https://modrinth.com/mod/ZHKrK8Rp) | Server Only | ✅ |
| [Discord Integration](https://modrinth.com/plugin/rbJ7eS5V) | Server Only | ✅ |
| [Terralith](https://modrinth.com/mod/8oi3bsk5) | Server Only | ✅ |
| [Let Me Despawn](https://modrinth.com/mod/vE2FN5qn) | Server Only | ✅ |
| [Lithium](https://modrinth.com/mod/gvQqBUqZ) | Client or Server | ✅ |
| [FerriteCore](https://modrinth.com/mod/uXXizFIs) | Client or Server | ✅ |
| [LazyDFU](https://modrinth.com/mod/hvFnDODi) | Client or Server | ✅ |
| [Alternate Current](https://modrinth.com/mod/r0v8vy1s) | Server Only | ✅ |
| [Sodium](https://modrinth.com/mod/AANobbMI) | Client Only | ✅ |
| [Sodium Extra](https://modrinth.com/mod/PtjYWJkn) | Client Only |  ✅ |
| [Reese's Sodium Options](https://modrinth.com/mod/Bh37bMuy) | Client Only | ✅|
| [LambDynamicLights](https://modrinth.com/mod/yBW8D80W) | Client Only | ✅ |
| [Continuity](https://modrinth.com/mod/1IjD5062) | Client Only | ✅ |
| [Indium](https://modrinth.com/mod/Orvt0mRa) | Client Only | ✅ |
| [Mod Menu](https://modrinth.com/mod/mOgUt4GM) | Client Only | ✅ |
| [Entity Culling](https://modrinth.com/mod/NNAgCjsB) | Client Only | ✅| 
| [Dynamic FPS](https://modrinth.com/mod/LQ3K71Q1) | Client Only | ✅ |
| [BetterF3](https://modrinth.com/mod/8shC1gFX) | Client Only | ✅ |
| [Enhanced Block Entities](https://modrinth.com/mod/OVuFYfre) | Client Only | ✅ |
| [Inventory Profile Next](https://modrinth.com/mod/O7RBXm3n) | Client Only | ✅ |
| [Zoomify](https://modrinth.com/mod/w7ThoJFB) | Client Only | ✅ |
| [Roughly Enough Items](https://modrinth.com/mod/nfn13YXA) | Client Only | ✅ |
| [Replay Mod](https://modrinth.com/mod/Nv2fQJo5) | Client Only | ✅ |
| [Replay Voice Chat](https://modrinth.com/mod/yI6ANuOK) | Client Only | ✅ |
| [Better Mound HUD](https://modrinth.com/mod/kqJFAPU9) | Client Only | ✅ |
| [MoreChatHistory](https://modrinth.com/mod/8qkXwOnk) | Client Only | ✅ |
| [Sound Physics Remastered](https://modrinth.com/mod/qyVF9oeo) | Client Only | ✅ |
| [Shulker Box Tooltip](https://modrinth.com/mod/qyVF9oeo) | Client Only | ✅ |
| [Better Statistic Screen](https://modrinth.com/mod/n6PXGAoM) | Client Only | ✅ |
| [Eating Animation](https://modrinth.com/mod/rUgZvGzi) | Client Only | ✅ |
| [Mouse Tweaks](https://modrinth.com/mod/aC3cM3Vq) | Client Only | ✅ |
| [Boat Item View](https://modrinth.com/mod/BdKIyOLe) | Client Only | ✅ |
| [Chat Heads](https://modrinth.com/mod/Wb5oqrBJ) | Client Only | ✅ |
| [Clickthrough](https://modrinth.com/mod/Z5b0cAlD) | Client Only | ✅ |
| [Clear Despawn](https://modrinth.com/mod/yoJJjRRE) | Client Only | ✅ |
| [Traveller's Titles](https://modrinth.com/mod/JtifUr64) | Client Only | ✅ |
| [Better Advancements](https://modrinth.com/mod/Q2OqKxDG) | Client Only | ✅ |
| [Item Swapper](https://modrinth.com/plugin/RPOSBQgq) | Client Only | ✅ |
| [Effective](https://modrinth.com/mod/pcPXJeZi) | Client Only | |
| [Status Effect Bars](https://modrinth.com/mod/x02cBj9Y) | Client Only | ✅ |
| [Clear Skies](https://modrinth.com/mod/xNK6XfRv) | Client Only | |
| [VTDownloader](https://modrinth.com/mod/1E2sq1cp) | Client Only | ✅ |
| [Not Enough Animations](https://modrinth.com/mod/MPCX6s5C) | Client Only | ✅ |
| [AntiGhost](https://modrinth.com/mod/Jw3Wx1KR) | Client Only | ✅ |
| [Stendhal](https://modrinth.com/mod/BWloQ9jK) | Client Only | ✅ |
| [Adaptive Tooltips](https://modrinth.com/mod/wFv8yCxM) | Client Only |  |
| [ExtraSounds](https://modrinth.com/mod/uPWCQrVL) | Client Only | |
| [Dark Loading Screen](https://modrinth.com/mod/h3XWIuzM) | Client Only | ✅ |
| [Cull Less Leaves](https://modrinth.com/mod/iG6ZHsUV) | Client Only | ✅ |
| [Advancement Plaques](https://modrinth.com/mod/9NM0dXub) | Client Only | ✅ |
| [AmbientSounds](https://modrinth.com/mod/fM515JnW) | Client Only | ✅ |
